...
import net.manaita_plus_neo.block.ManaitaCraftingBlock;
...
@OnlyIn(Dist.CLIENT)
public class RenderCraftingManaitaBlockEntity implements BlockEntityRenderer<ManaitaCraftingBlockEntity> {
    private final EntityRenderDispatcher entityRenderer;
    private final ItemStack stack;

    public RenderCraftingManaitaBlockEntity(BlockEntityRendererProvider.Context context) {
        this.entityRenderer = context.getEntityRenderer();
        stack = new ItemStack(ModItems.CRAFTING_BLOCK.get());
        CompoundTag tag = new CompoundTag();
        stack.set(DataComponents.CUSTOM_DATA, CustomData.of(tag));
    }

    @Override
    public void render(ManaitaCraftingBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        if (block instanceof ManaitaCraftingBlock) {
    ...
    @Override
    public boolean shouldRenderOffScreen(ManaitaCraftingBlockEntity blockEntity) {
    ...
    @Override
    public boolean shouldRender(ManaitaCraftingBlockEntity blockEntity, Vec3 cameraPos) {
